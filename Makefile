...
test:
	...
	@if [ -f services/voice-gateway/gradlew ]; then \
		./scripts/with-gradle-jdk.sh services/voice-gateway ./gradlew test; \
	else \
		echo "[voice-gateway] No tests yet (Gradle not initialized)"; \
	fi
	@if [ -f apps/android/gradlew ]; then \
		./scripts/with-android-env.sh apps/android ./gradlew test; \
	else \
...
lint:
	...
	@if [ -f services/voice-gateway/gradlew ]; then \
		./scripts/with-gradle-jdk.sh services/voice-gateway ./gradlew ktlintCheck 2>/dev/null || echo "[voice-gateway] ktlint not configured yet"; \
	else \
		echo "[voice-gateway] No linter yet (Gradle not initialized)"; \
	fi
	@if [ -f apps/android/gradlew ]; then \
		./scripts/with-android-env.sh apps/android ./gradlew lint 2>/dev/null || echo "[android] lint not configured yet"; \
	else \
